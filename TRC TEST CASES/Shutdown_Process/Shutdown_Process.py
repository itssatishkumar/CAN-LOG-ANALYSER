...
# -------------------------------------------------------
# TIMESTAMP PARSING (with proper fractional second support)
# -------------------------------------------------------
def parse_timestamp(ts_str: str) -> float:
    """Convert timestamp like '23-05-2026 03:23:59.7385' to absolute seconds since epoch"""
    try:
        if ' ' in ts_str:
            if '.' in ts_str:
...
# -------------------------------------------------------
# PART 1: SHUTDOWN DETECTION (1840F400 is primary reference)
# -------------------------------------------------------
def detect_shutdown_events(frames):
    """Detect shutdown events - ALWAYS uses 1840F400 as primary reference for ACK timing"""
    events = []
    prev_state = None
    prev_ts_sec = None
    prev_ts_str = None
    ...
    for fr in frames:
        ...
        if fr.can_id == ID_VEHICLE:
            ...
            if prev_state in (0x02, 0x01) and state == 0x00:
                # Vehicle transition found (running -> off)
                if pending_shut is not None:
                    # We have a pending shutdown command - valid pair
        ...
        # Shutdown command detection (1840F400)
        ...
        # Handle incomplete events (timeout) - Shutdown command only
        ...
        # Handle incomplete events (timeout) - Vehicle transition only with detailed BMS analysis
        if pending_veh is not None and (ts_sec - pending_veh["ts_sec"]) > 3.0:
            veh_transition_ts = pending_veh["ts_sec"]
            
            # Check if BMS (0x12B) was present BEFORE vehicle transition
            bms_before = False
            for fr2 in frames:
            ...
            # Check if BMS (0x12B) appears within 2 seconds AFTER vehicle transition
            bms_after = False
            for fr2 in frames:
                if fr2.ts_sec < veh_transition_ts:
                    continue
            ...
            if bms_before and not bms_after:
                # Scenario 1: CAN bus issue - BMS stopped after transition
                events.append({
                ...
                # Scenario 2: BMS dead - No BMS ever
                events.append({
                    "shut_ts": pending_veh["ts_str"],
                    "shut_ts_sec": pending_veh["ts_sec"],
                    "shut_cmd_ts_sec": None,
                ...
                # Scenario 3: VCU forgot shutdown - BMS alive
                events.append({
                    "shut_ts": pending_veh["ts_str"],
                    "shut_ts_sec": pending_veh["ts_sec"],
                    "shut_cmd_ts_sec": None,
...
# -------------------------------------------------------
# PART 2: SOC RESTORE ANALYSIS (with ACK optional logic)
# -------------------------------------------------------
def analyze_soc_restore(frames, shutdown_events):
    ...
    for ev in shutdown_events:
        if ev.get("shut_cmd_missing", False):
            # Determine the appropriate remark based on scenario
        ...
        # ACK OPTIONAL LOGIC - Based on MCU ranges
        if ack_status == "MISSING":
            # MCU < 90: ACK not required
            ...
            # MCU between 90 and 200: ACK optional
...
def save_results(cycles, filepath):
    ...
    print("\n" + "="*100)
    print("SHUTDOWN PROCESS ANALYSIS RESULTS")
    print("="*100)
    ...
    for i, c in enumerate(cycles, 1):
        ...
        print(f"\n--- Cycle {i} ---")
        print(f"  Shutdown Timestamp: {c['shut_ts']}")
        print(f"  Vehicle Off Time: {c['veh_time']}s" if c['veh_time'] else "  Vehicle Off Time: N/A")
        print(f"  Time Between Cmd & Transition: {c['time_between']}s" if c['time_between'] else "  Time Between Cmd & Transition: N/A")
        print(f"  Start SOC: {c['Start_SoC']}%" if c['Start_SoC'] is not None else "  Start SOC: N/A")
        print(f"  Reflect SOC: {c['Reflect_SoC']}%" if c['Reflect_SoC'] is not None else "  Reflect SOC: N/A")
        print(f"  Delta SOC: {c['Delta']}%" if c['Delta'] is not None else "  Delta SOC: N/A")
        print(f"  SOC Result: {c['SoC_Result']}")
        print(f"  MCU After Shutdown: {c['MCU_After_Shutdown']}" if c['MCU_After_Shutdown'] is not None else "  MCU After Shutdown: N/A")
        print(f"  MCU Reset: {c['MCU_Reset']}")
        print(f"  MCU Reboot: {c['MCU_Reboot_Complete']}")
        print(f"  ACK: {ack_display}")
        print(f"  Final: {c['Final']}")
        print(f"  Remark: {c['Remark']}")
    ...
    with open(results_path, "w", encoding="utf-8") as jf:
        json.dump({"Result": overall_result, "Total_Cycles": len(cycles)}, jf, indent=4, ensure_ascii=False)
...
def save_plot_png(cycles):
    ...
    if not cycles:
        ...
        for idx, c in enumerate(cycles, start=1):
            ...
            # Wrap long remarks to multiple lines (45 characters per line)
    ...
    # Calculate dynamic height based on number of lines in remarks
    max_remark_lines = 1
    for row in rows:
        remark = row[9]
        if remark and '\n' in remark:
    ...
    # Adjust height: base height + extra for multi-line remarks
    ...
    table = ax.table(cellText=rows,colLabels=headers,colWidths=col_widths,cellLoc="center",bbox=[0.0, 0.0, 1.0, 1.0])
...
def main():
    if len(sys.argv) < 2:
        ...
        root = tk.Tk()
        root.withdraw()
        filepath = filedialog.askopenfilename(title="Select TRC File", filetypes=[("TRC files", "*.trc"), ("All files", "*.*")])
        if not filepath:
    ...
    print(f"\nFound {len(shutdown_events)} shutdown events")
    ...
    for parent in [p] + list(p.parents):
        ...
        if history.exists() and history.is_dir():
            ...
            with open(history_file, "w", encoding="utf-8") as f:
                f.write(text)
            print(f"\nHistory file saved -> {history_file}")
            print(f"Content: {text}")
            break
    
    print("\nPROGRESS 100.0", flush=True)
