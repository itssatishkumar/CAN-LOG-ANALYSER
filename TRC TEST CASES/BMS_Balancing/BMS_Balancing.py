...
# Fast TRC -> frames
# -------------------------------------------
def parse_trc_fast(path, progress_cb=None):
    """
...
# -------------------------------------------
def decode_frames_fast(frames, dbc, progress_cb=None):
    """
    High-speed decode:
    ...
    for row_idx, (ts_raw, t, cid, data) in enumerate(frames, start=0):
        if progress_cb:
            progress_cb(row_idx + 1)
        # Base columns
...
def main():
    ...
    frames = parse_trc_fast(trc, progress_cb=parse_cb)

    decode_cb = make_progress_cb(len(frames), start=60.0, end=98.0) if frames else None
    df = decode_frames_fast(frames, dbc, progress_cb=decode_cb)
